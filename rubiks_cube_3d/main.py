...
class Game(Ursina):
    def __init__(self):
        ...
        Entity(
            model="sphere", scale=100, texture="textures/sky0", double_sided=True
        )  # sky
        EditorCamera()
    ...
    def load_game(self):
        ...
        self.CUBES = [
            Entity(model=self.model, texture=self.texture, position=pos)
    ...
    def random_state(self, rotations=3):
        [
            self.rotate_side_without_animation(random.choice(list(self.rotation_axes)))
    ...
    def toggle_game_mode(self):
        ...
        msg = dedent(
            f"{'ACTION mode ON' if self.action_mode else 'VIEW mode ON'}"
            f" (to switch - press middle mouse button)"
        ).strip()
    ...
    def create_cube_positions(self):
        ...
        self.SIDE_POSITIONS = (
            self.LEFT | self.BOTTOM | self.FACE | self.BACK | self.RIGHT | self.TOP
        )
    ...
    def input(self, key):
        if key in "mouse1 mouse3" and self.action_mode and self.action_trigger:
            for hitinfo in mouse.collisions:
                ...
                if (
                    key == "mouse1"
                    and collider_name in "LEFT RIGHT FACE BACK"
                    or key == "mouse3"
                    and collider_name in "TOP BOTTOM"
                ):
